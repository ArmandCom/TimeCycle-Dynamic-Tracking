...
def _make_radial_window(width, height, cx, cy, fn, window_width=10.0):
    ...
    ys = torch.arange(height).view((height, 1)).repeat(channels, 1, width).float().cuda()
    delta_xs = xs - cx
    delta_ys = ys - cy
...
class SoftArgmax1D(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        indices = torch.arange(start=self.base_index, end=end_index, step=self.step_size).float().cuda()
