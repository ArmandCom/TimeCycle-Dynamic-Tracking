...
class TrajectoryMultiple(data.Dataset):
  def __init__(self, is_train, traj_length, dset_path = '', generate=False):
    ...
    # For generating data
    self.n_forks = 2
    self.n_traj = 1 # Number of coexisting trajectories
    ...
    self.pole_low, self.pole_high = 0.3, 1.5
    self.clean = True
    ...
    self.added_noise_std = 0.2   # Variance of the 0-mean gaussian noise in the measurements
  ...
  def generate_dataset(self, root='', training_samples = 1e3, testing_samples = 5e2):
    ...
    n_train_iters = training_samples
    n_test_iters = testing_samples
...
def main():
  ...
  traj_length = 9
  traj2D = TrajectoryMultiple(True, traj_length=traj_length)
