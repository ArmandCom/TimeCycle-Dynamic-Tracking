...
def distance_closest_sequence(S, Sall):
    assert len(S.shape) == 4 and len(Sall.shape) == 4
    se_dists = (Sall-S)**2
    nn_se_dist = se_dists.min(-2)[0].unsqueeze(-2)
    return nn_se_dist

def weighted_distance_closest_sequence(S, Sall):
    assert len(S.shape) == 4 and len(Sall.shape) == 4
    se_dists = (Sall-S)**2
    nn = se_dists.min(-2)
    nn_dist = nn[0].unsqueeze(-2)
    nn_idx = nn[1].unsqueeze(-2)
    nn = torch.gather(Sall, -2, nn_idx)
    nn_dist = nn_dist / nn
    # square?
    return nn_dist
