...
def gram_matrix(y, delta=0, unbiased = False, diff = False):
...
def gram_matrix_sized(y, delta=0, size=None, unbiased = False, diff = False):
...
def JBLDLoss_increasing(S, delta, sz=None):
    ...
    for i in range(2):
        if i!=0:
            idx_rev = torch.arange(S.shape[-1]-1, -1, -1)
            S = S[..., idx_rev]
        for t in range(S.shape[-1] - sz + 1):
            G1 = gram_matrix(S[..., 0:sz], delta=delta)
            G2 = gram_matrix_sized(S[..., 0:t+sz],  delta=delta, size=((sz+1)/2))
            jblds.append(JBLD(G1, G2))
...
def comp_gram_matrix(y_ini, delta=0, unbiased = False, diff = False, pairwise=None):
    ...
    if delta!=0:
        G = G + delta * torch.eye(nc, nc)#.cuda()
        # if pairwise=='inter' or pairwise=='intra':
        #     G_int = G_int + delta * torch.eye(nc, nc).cuda(gpu_id)
    return G, G_int

def trace_G(G):
    # Compute and return during generation of G from y?
    # TODO: get values in diagonal with indexes from arange(G.shape[-1]) and a sum --> easy peasy
    return None
