...
# from hankel import gram_matrix, trace_G, JBLDLoss, JBLDLoss_rolling, JBLDLoss_rolling_to_all, JBLDLoss_increasing
...
class TrajectoryMultiple(data.Dataset):
  def __init__(self, traj_length, dset_path = '', generate=False):
    ...
    self.max_order = 1      # Maximum order of the LTI systems generated IRs #TODO: this m**f** doesnt work
    self.missing_data_perc = 0  # How many missing points in the trajectory
    self.pole_low, self.pole_high = 0.4, 1.1
    self.clean = False #TODO: change to false?
    ...
    self.centroid_min_randint, self.centroid_max_randint = -5, 5
    self.std_max_rand = 1.2 # std of the gaussian by which we generate the std of the gaussian around a centroid
    ...
    self.loss_dynamics_jbld = lambda S: JBLDLoss_increasing(S, delta=self.delta, sz=3)
  ...
  def get_random_trajectory(self, traj_length=None, pole_limits = None, n_realizations = 2):
    ...
    for tr in range(self.n_traj):
      ...
      y[:, 0:self.max_order, :] = np.random.uniform(0, 70, n_realizations * self.max_order * self.n_traj) \
      ...
      traj_clean.append(np.copy(y[...,tr]))
    ...
    if branching:
      ...
      if np.random.uniform(0,1) < 0.5:
        # Reverse second trajectory
        idx_rev = torch.arange(trajs.shape[-1]-1, -1, -1)
        trajs[1:] = trajs[1:,...,idx_rev]
      else:
        #Add offset to the non-original trajectories
        trajs[1:] += np.random.uniform(2, 6)

      # Only works with one fork
      if num_forks ==1:
        fork = forks[0]
        dists = trajs[0:1, ..., fork-1:fork] - trajs[1:, ..., fork-1:fork]
        trajs[1:] += dists
      ...
      # Note: PER MARINA
      if self.Marina:
        fork = forks[0] # we assume there's only one fork
        trajectories[..., :fork] = trajectories[..., 0:1, :fork]

        if self.save_test:
          for k in range(self.k):
            fig = plt.plot(trajectories[0, k])
            plt.savefig('example_traj_marina')
          plt.close()
      ...
      #       trajectories[c, :, tr].sort()
        ...
        # Note: If first 3 points are known, therefore equal
        trajectories[..., :3] = trajectories[..., :1, :3]
      ...
      if self.save_test:
        for k in range(self.k):
          if k == 0:
            fig = plt.plot(trajectories[0, k], 'bo--')
          if k == 3:
            fig = plt.plot(trajectories[0, k], 'gx--')
          else:
            fig = plt.plot(trajectories[0, k])
        ...
        # print(np.mean(np.abs(trajectories)), trajectories.shape, centroids.mean(), centroids_false.mean())
    # else:
    #   '''
    #   --------------------------------------------------------------------------------------------
  ...
  def generate_dataset(self, traj_length, root='', training_samples = 1e3, testing_samples = 5e2):
    ...
    for i in range(n_train_iters):
      train_data = self.get_random_trajectory(traj_length = traj_length)
      train_data_list.append(train_data)
    for i in range(n_test_iters):
      test_data = self.get_random_trajectory(traj_length = traj_length)
    ...
    if self.save:
      ...
      train_dat_name = os.path.join(root, 'train_dat_sys.npy')
      test_dat_name = os.path.join(root, 'test_dat_sys.npy')
  ...
  def load_data(self, is_train):
    ...
    if is_train:
      data_name = 'train_dat_sys.npy'
    else:
      data_name = 'test_dat_sys.npy'
